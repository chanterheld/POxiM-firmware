...
static const int32_t stage7_coeff[] = {5120, 53504, 110336, -148992, -819968, -543232, 2222336, 4315392, -1463808, -12609280, -9195520, 19376640, 38183424, -4659968, -83977472, -71265280, 128718848, 417668352, 556807936, 417668352, 128718848, -71265280, -83977472, -4659968, 38183424, 19376640, -9195520, -12609280, -1463808, 4315392, 2222336, -543232, -819968, -148992, 110336, 53504, 5120};

//#define c0	(stage7_coeff[0])
//#define c36	(stage7_coeff[36])

#define FILTER_NAME				stage7_ir_rom_opt
...
DEC_FIR_FILTER_ADVANCE(FILTER_NAME, filter_input_type, filter_output_type){
	...
	if(output_cycle == 1){
		...
		for(uint8_t i = 2; i < 36; i+=2){
			int64_t temp2 = 	input * (stage7_coeff[i]) + CONCAT2(FILTER_NAME,_filter_memory)[(i/2)];
			CONCAT2(FILTER_NAME,_filter_memory)[(i/2) - 1] = ((filter_output_type *)(&temp2))[0];
		}
...
//		FILTER_LINE_MEM_SAVE_MOVE_CAST(FILTER_NAME, 34, filter_output_type);
		...
		for(uint8_t i = 1; i < 36; i+=2){
			int64_t temp2 = input * (stage7_coeff[i]) + CONCAT2(FILTER_NAME,_filter_memory)[((i - 1)/2)];
			CONCAT2(FILTER_NAME,_filter_memory)[((i - 1)/2)] = ((filter_output_type *)(&temp2))[0];
		}
