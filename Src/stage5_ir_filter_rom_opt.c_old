...
static const int32_t stage5_coeff[] = {4534784, 10265344, -26586880, -84064768, 65555456, 500018688, 765431552, 500018688, 65555456, -84064768, -26586880, 10265344, 4534784};

#define FILTER_NAME				stage5_ir_rom_opt
...
DEC_FIR_FILTER_ADVANCE(FILTER_NAME, filter_input_type, filter_output_type){
	...
	if(output_cycle == 1){
		...
		for(uint8_t i = 2; i < FILTER_SIZE; i+=2){
			int64_t temp2 = 	input * (stage5_coeff[i]) + CONCAT2(FILTER_NAME,_filter_memory)[(i/2)];
...
//		FILTER_LINE_MEM_SAVE_MOVE_CAST(FILTER_NAME, 10, filter_output_type);
		...
		for(uint8_t i = 1; i < FILTER_SIZE; i+=2){
			int64_t temp2 = input * (stage5_coeff[i]) + CONCAT2(FILTER_NAME,_filter_memory)[((i - 1)/2)];
