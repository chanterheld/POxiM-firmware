...
static const int32_t stage6_coeff[] = {-560384, -2084096, 1233920, 15193088, 11409408, -49801472, -85861120, 95791104, 472342016, 678930944, 472342016, 95791104, -85861120, -49801472, 11409408, 15193088, 1233920, -2084096, -560384};

#define FILTER_NAME				stage6_ir_rom_opt
...
DEC_FIR_FILTER_ADVANCE(FILTER_NAME, filter_input_type, filter_output_type){
	...
	if(output_cycle == 1){
		...
		for(uint8_t i = 2; i < FILTER_SIZE; i+=2){
			int64_t temp2 = 	input * (stage6_coeff[i]) + CONCAT2(FILTER_NAME,_filter_memory)[(i/2)];
...
//		FILTER_LINE_MEM_SAVE_MOVE_CAST(FILTER_NAME, 16, filter_output_type);
		...
		for(uint8_t i = 1; i < FILTER_SIZE; i+=2){
			int64_t temp2 = input * (stage6_coeff[i]) + CONCAT2(FILTER_NAME,_filter_memory)[((i - 1)/2)];
